...
| UploadWorkflow / UploadLifecyclePanel | Upload and active-file lifecycle | current file/version, job state, replace/requeue actions | intake, replacement, requeue | progress semantics and error/success live regions | single-column mobile | `apps/web/components/domain/upload` | document intake/detail, replace PDF |
