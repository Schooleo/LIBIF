...
| 3 — Parallel P0 build | **In progress.** Member C's C7-001/C7-002 renderer baseline is already merged; Member D's users read API is live and product-settings persistence is implemented while A/B continue their owning lanes. | **A7-001 → A7-004 → A7-002**; implement guarded manifest/page delivery, required audit/rate/scrape enforcement, then canvas integration. **A7-003** may run in parallel; finish **A7-005** before the P0 gate. | **B7-001 → B7-002**; implement direct published-only detail, then catalogue URL-state discovery/navigation. Publish tested detail DTO behavior to A. | **C7-001/C7-002 merged**; support only renderer integration defects until committed risk facts unblock **C7-003**. | **D7-001 backend slice live** (`GET /api/admin/users`, `GET /api/admin/users/:userId`); **D7-004 persistence complete**, but `GET/PATCH /api/admin/settings/general` remains gated on Member A's tested capability source. Keep generated-client/web closure deferred to D7-005. |
...
- **Member C:** C7-001/C7-002 are merged into `dev` and this branch; only fix renderer integration defects until committed risk facts unblock C7-003.
- **Member D:** D7-001 read routes are live and D7-004 product-settings persistence is implemented. Keep the settings controller non-live until Member A supplies the tested capability source; retain integration ownership and defer generated-client refresh plus admin web pages to D7-005.
...
- [ ] Wave 3 Reader/catalogue integration evidence remains pending beyond the merged C renderer baseline, Member D users read API, and capability-gated settings persistence.
